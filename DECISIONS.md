...
## 2026-06-15 — Memes freq gate: LIKE → semantic vec match
- (verified) `_events_semantic_count_14d` replaces `_events_like_count_14d` as memes frequency gate. max(LIKE, vec KNN cosine>=0.65) so paraphrases count (灭绝师太/师太/A&P教授 = same topic) and partial vec coverage doesn't regress. LIKE kept as floor, not removed.
- (reasoned) Cosine 0.65 chosen looser than dedup 0.85 — freq gate asks "mentioned the topic" not "exact duplicate". K=200 generous ceiling; daily candidates volume is single-digit.
