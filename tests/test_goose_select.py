"""Tests for marrow/goose_select.py — deterministic bits mocked."""
...
from marrow import goose_select, storage
...
def test_parse_day_block_extracts_quotes(tmp_path, monkeypatch):
    monkeypatch.setattr(goose_select, "_QUOTE_DIR", tmp_path / "语录")
    _make_monthly(tmp_path, "2026-05-01", [
        "- `00:25` 妈这版配色讲究嘎",
        "- `00:26` 妈这改对齐了嘎",
    ])
    result = goose_select._parse_day_block("2026-05-01")
    assert result == ["妈这版配色讲究嘎", "妈这改对齐了嘎"]


def test_parse_day_block_missing_file_returns_empty(tmp_path, monkeypatch):
    monkeypatch.setattr(goose_select, "_QUOTE_DIR", tmp_path / "语录")
    result = goose_select._parse_day_block("2026-05-99")
    assert result == []
...
def test_parse_day_block_skips_banner(tmp_path, monkeypatch):
    ...
    result = goose_select._parse_day_block("2026-05-02")
    assert result == ["嘎"]
...
def test_select_single_candidate_skips_llm(tmp_path, monkeypatch, db):
    ...
    with patch.object(goose_select, "_call_haiku") as mock_haiku:
        result = goose_select.select_quote_for_date(db, "2026-05-03")
...
def test_select_haiku_called_for_multiple_candidates(tmp_path, monkeypatch, db):
    ...
    with patch.object(goose_select, "_call_haiku", return_value="quote B") as mock_h:
        result = goose_select.select_quote_for_date(db, "2026-05-04")
...
def test_select_haiku_fallback_on_none(tmp_path, monkeypatch, db):
    ...
    with patch.object(goose_select, "_call_haiku", return_value=None):
        result = goose_select.select_quote_for_date(db, "2026-05-05")
...
def test_call_haiku_out_of_candidates_falls_back_to_longest():
    ...
    with patch("marrow.goose_select.LLMClient", return_value=mock_client):
        result = goose_select._call_haiku(["short", "longer candidate here"])
    assert result == "longer candidate here"


def test_call_haiku_llm_error_returns_none():
    with patch("marrow.goose_select.LLMClient", side_effect=Exception("fail")):
        result = goose_select._call_haiku(["a", "b"])
    assert result is None
...
def test_upsert_insert_new_row(db):
    goose_select._upsert(db, "2026-05-10", "test quote")
    row = db.execute(
...
def test_upsert_replace_existing_row(db):
    ...
    db.commit()
    goose_select._upsert(db, "2026-05-11", "new quote")
    rows = db.execute(
...
def test_select_no_candidates_returns_none(tmp_path, monkeypatch, db):
    ...
    (tmp_path / "语录").mkdir()
    result = goose_select.select_quote_for_date(db, "2026-06-01")
    assert result is None
