...
- **`/info` field redesign** — Lumi wants real 5h usage %, not "hours until reset". cc 2.1.159 stream-json does NOT expose `percentUsed` (only `resetsAt` + `isUsingOverage`); cc's own statusline reads quota via internal endpoint not exposed here. Options surfaced (pick later):
  - (A) display cumulative `total_cost_usd` + reset countdown — cheap, immediate
  - (B) short `claude -p "/status"` subprocess to parse cc's own status — ~3-5s blocking + extra LLM call + may not work in -p mode
  - (C) wait for cc to re-add `percentUsed` in stream-json
  - **(D) undocumented OAuth endpoint `https://api.anthropic.com/api/oauth/usage`** — bearer from `~/.claude/.credentials.json` + `anthropic-beta: oauth-2025-04-20` header; returns 5h % + 7d % + resetsAt; **this is the same source cc statusline uses**. Risk: aggressively 429-rate-limited (community report: nearly impossible to poll); requires ≥5min in-memory cache + graceful fallback to (C)'s `?(5h)/?(7d)` on 429. Unofficial — could change without notice. Reference impl: github.com/ohugonnot/claude-code-statusline.
  - **Verdict 2026-06-02**: dropped for now — Lumi's wx volume doesn't approach quota so the display isn't worth the 429 risk + unofficial-endpoint exposure. Revisit if usage ever pinches.
  - **ctx display bug fixed (`d0f8bf5`)** — old `/info` summed cumulative `input + cache_read + cache_creation + output` across turns, double-counting `cache_read` every turn (35k ctx rendered as 75-150k). Now reads last assistant turn's `input + cache_read + cache_creation` snapshot — matches cc statusline's "Context N.Nk".
  - **model display fixed** — `/info` model field now (1) parses `model` from `system/init` event so bridge no longer needs an explicit `--model` to know what cc is running, (2) preserves `[1m]`/`[200k]` context suffix in display (`Opus 4.7 [1M]` not `Opus 4.7`), (3) shows `?` instead of misleading `default` when state is empty.
