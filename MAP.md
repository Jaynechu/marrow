...
- turn_inject 100k 亮牌 (hooks.py) — `_cortex_show_context(tpath)`, gated `enabled()`: cortex-only (checks MARROW_CORTEX itself) window-occupancy nudge at `[cortex_rotate].show_tokens` (100k soft, ahead of the 150k fuse; 0 = off), text hardcoded in cortex_bridge.py.
