...
- **events_vec_embedder_provenance** — events_vec lacks embedder-id/dim columns (review #6). DEFERRED to Phase 2 recall-module first build (DESIGN:259): embedder deferred (fork #1); add provenance with embedder so a model swap re-embeds without a base-schema rewrite (goal 1/7). Fusion ref: Ombre-Brain github.com/P0luz/Ombre-Brain (weight-pool single decayed score, not RRF), cyberboss github.com/WenXiaoWendy/cyberboss.
