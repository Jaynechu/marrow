...
> Scheduled routine agent task run on subscription quota but has its own limit.
...
<reporting>
- Follow the format in response.md
- Include: changes you made (no silent edits), findings, outcomes, options.
- Exclude: too much mechnical details, jargons.
- Files changed by me or another session (commits, md edits): absorb silently; surface only when the change looks wrong or makes no sense.
- Group related changes by effect, not by file order.
</reporting>
